...
def remove_cryosparc_leading_uid(df, inplace=False):
    """
    Remove leading UIDs generated by cryoSPARC from path names if they exist.

    For example:
    Before: J79/imported/002635156430113862662_14sep05c_00024sq_00003hl_00002es_frames.mrc
    After: J79/imported/14sep05c_00024sq_00003hl_00002es_frames.mrc
    """
    df = df if inplace else df.copy()
    # always at the beginning of the path (given we're using the basename)
    # 21 numbers, followed by an underscore
    uid_match_re = r'^\d{21}_'
    path_columns = [
        star.Relion.IMAGE_NAME,
        star.UCSF.IMAGE_BASENAME,
        star.Relion.MICROGRAPH_NAME,
        star.UCSF.MICROGRAPH_BASENAME]
    ...
    for column_name in path_columns:
        if column_name in df:
            # check if there are any leading UIDs in any of the paths in this column
